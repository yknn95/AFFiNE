...
test('NativeProviderAdapter streamObject should fallback to image tool on empty native response', async t => {
  const adapter = new NativeProviderAdapter(
    () =>
      stream(() => [
        { type: 'message_start', model: 'gpt-5.5' },
        { type: 'provider_selected', provider_id: 'openai-default' },
        { type: 'done', finish_reason: 'stop' },
      ]),
    {
      fallbackImageTool: {
    ...
    adapter.streamObject(
      {
        model: 'gpt-5.5',
        stream: true,
        messages: nativeMessages(nativeUserText('生成一张相关图')),
      },
      undefined,
      [
        {
  ...
  t.deepEqual(
    ...
    ['tool-call:image_generate', 'tool-result:image_generate']
  );
  const toolCall = events[0];
  const toolResult = events[1];
  t.is(toolCall?.type, 'tool-call');
  t.is(toolResult?.type, 'tool-result');
  if (toolCall?.type !== 'tool-call' || toolResult?.type !== 'tool-result') {
    return;
  }
  t.is(toolResult.toolName, 'image_generate');
  t.deepEqual(toolResult.args, { prompt: '生成一张相关图', count: 1 });
  t.deepEqual(toolResult.result, {
    prompt: '生成一张相关图',
    images: [
      {
        url: 'https://cdn.example.com/generated.png',
  ...
  t.is(toolCall.toolCallId, toolResult.toolCallId);
});
